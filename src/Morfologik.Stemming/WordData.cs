...
namespace Morfologik.Stemming
{
    ...
    /// <para/>
    /// Instances of this class are reused (values
    /// returned from <see cref="Stem"/>, <see cref="Word"/>
    ...
    /// yourself and copy the associated data, call the <see cref="WordData(WordData)"/>
    /// costructor to make a clone, or create
    /// strings (they are immutable) using <see cref="Stem"/> and then
    /// <see cref="ReadOnlyMemory{T}.ToString()"/>.
    /// <para/>
    ...
    public sealed class WordData
    {
        ...
        internal WordData(IWordDataStorage storage)
        {
            Debug.Assert(storage is not null);
            this.storage = storage!;
        }

        internal WordData(IWordDataStorage storage, int index)
        {
            Debug.Assert(storage is not null);
            this.storage = storage!;
            this.index = index;
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="WordData"/> class that contains
        /// the values copied from the specified <see cref="WordData"/>.
        /// </summary>
        /// <param name="wordData">The <see cref="WordData"/> to copy.</param>
        ...
        // Morfologik.Stemming: Converted Clone() to a copy constructor
    }
}
